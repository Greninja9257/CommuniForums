...
router.get('/profile/:id', (req, res) => {
  ...
  // Activity stats
  const thanksGivenRecent = db.prepare(
    "SELECT COUNT(*) as c FROM thanks WHERE giver_id = ? AND created_at > datetime('now', '-30 days')"
  ).get(user.id).c;

  const postsRecent = db.prepare(
    "SELECT COUNT(*) as c FROM posts WHERE author_id = ? AND created_at > datetime('now', '-30 days')"
  ).get(user.id).c;

  let rankOverrideBy = null;
  if (user.rank_override_by) {
    rankOverrideBy = db.prepare('SELECT id, username FROM users WHERE id = ?').get(user.rank_override_by);
  }
...
router.post('/settings', requireAuth, async (req, res) => {
  ...
  if (bio !== undefined) {
    ...
    db.prepare('UPDATE users SET bio = ? WHERE id = ?').run(cleanBio, res.locals.currentUser.id);
  }

  if (avatar) {
    // Basic URL validation
    if (avatar.match(/^https?:\/\/.+\..+/)) {
      db.prepare('UPDATE users SET avatar = ? WHERE id = ?').run(avatar, res.locals.currentUser.id);
    }
  }

  // Password change
  if (current_password && new_password) {
    const user = db.prepare('SELECT password_hash FROM users WHERE id = ?').get(res.locals.currentUser.id);
    ...
    db.prepare('UPDATE users SET password_hash = ? WHERE id = ?').run(hash, res.locals.currentUser.id);
  }

  res.render('users/settings', { title: 'Settings', error: null, success: 'Settings updated successfully!' });
});

// GET /users/members (member list)
...
// POST /users/profile/:id/rank (highest rank only)
router.post('/profile/:id/rank', requireAuth, (req, res) => {
