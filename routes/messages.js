...
// POST /messages/send
router.post('/send', requireAuth, blockBanned, (req, res) => {
  ...
  const recipient = db.prepare('SELECT id FROM users WHERE username = ?').get(to.trim());
  if (!recipient) {
    return res.render('messages/compose', {
...
router.get('/:id', requireAuth, (req, res) => {
  ...
  if (message.receiver_id === res.locals.currentUser.id && !message.is_read) {
    db.prepare('UPDATE private_messages SET is_read = 1 WHERE id = ?').run(message.id);
  }
...
router.post('/:id/delete', requireAuth, (req, res) => {
  ...
  if (message.sender_id === res.locals.currentUser.id) {
    db.prepare('UPDATE private_messages SET sender_deleted = 1 WHERE id = ?').run(message.id);
  } else if (message.receiver_id === res.locals.currentUser.id) {
    db.prepare('UPDATE private_messages SET receiver_deleted = 1 WHERE id = ?').run(message.id);
  }
