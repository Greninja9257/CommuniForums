...
async function attachUser(req, res, next) {
  if (req.session && req.session.userId) {
    let user = null;
    try {
      user = await db.prepare('SELECT * FROM users WHERE id = ?').get(req.session.userId);
    } catch (error) {
      user = null;
    }
    ...
    if (user) {
      ...
      try {
        ...
        // Keep user attached even if trust refresh fails.
      }
      ...
      let notifCount = { count: 0 };
      let pmCount = { count: 0 };
      try {
        notifCount = await db.prepare(
